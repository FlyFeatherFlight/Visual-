//------------------------------------------------------------------------------
// <auto-generated>
//    此代码是根据模板生成的。
//
//    手动更改此文件可能会导致应用程序中发生异常行为。
//    如果重新生成代码，则将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

using System;
using System.Collections.Generic;

public partial class 营销_接待计划
{
    public 营销_接待计划()
    {
        this.财务_接待报销 = new HashSet<财务_接待报销>();
        this.营销_接待计划费用预估 = new HashSet<营销_接待计划费用预估>();
        this.营销_接待计划明细 = new HashSet<营销_接待计划明细>();
        this.营销_接待来宾信息 = new HashSet<营销_接待来宾信息>();
        this.营销_接待来访主要事项 = new HashSet<营销_接待来访主要事项>();
    }

    public int ID { get; set; }
    public string 部门 { get; set; }
    public System.DateTime 申请日期 { get; set; }
    public int 申请人ID { get; set; }
    public string 航班号 { get; set; }
    public Nullable<System.DateTime> 抵达日期 { get; set; }
    public Nullable<System.DateTime> 离开日期 { get; set; }
    public string 入住酒店 { get; set; }
    public Nullable<decimal> 费用总预算 { get; set; }
    public string 主要沟通内容 { get; set; }
    public string 配合部门 { get; set; }
    public int 提交人ID { get; set; }
    public System.DateTime 提交日期 { get; set; }
    public Nullable<int> 更新人ID { get; set; }
    public Nullable<System.DateTime> 更新日期 { get; set; }
    public Nullable<bool> 审核状态 { get; set; }
    public Nullable<int> 审核人ID { get; set; }

    public virtual ICollection<财务_接待报销> 财务_接待报销 { get; set; }
    public virtual 系统用户 系统用户 { get; set; }
    public virtual 系统用户 系统用户1 { get; set; }
    public virtual 系统用户 系统用户2 { get; set; }
    public virtual 系统用户 系统用户3 { get; set; }
    public virtual ICollection<营销_接待计划费用预估> 营销_接待计划费用预估 { get; set; }
    public virtual ICollection<营销_接待计划明细> 营销_接待计划明细 { get; set; }
    public virtual ICollection<营销_接待来宾信息> 营销_接待来宾信息 { get; set; }
    public virtual ICollection<营销_接待来访主要事项> 营销_接待来访主要事项 { get; set; }
}
