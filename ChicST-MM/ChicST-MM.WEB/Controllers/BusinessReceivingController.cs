...
namespace ChicST_MM.WEB.Controllers
{
    ...
    public class BusinessReceivingController : Controller
    {
        ...
        public ActionResult BusinessRec_PlanAdd(BusinessReceiving_PlansViewModel businessReceiving_PlansViewModel)
        {
            ...
            model.车辆配备= businessReceiving_PlansViewModel.车辆配备;
        ...
        /// <returns></returns>
        public ActionResult ReceivingRecords_DetailsEditView(int id) {
        ...
        public ActionResult CheckRecord(int id, string checkString)
        {
            ...
            if (model == null||model.审核人ID!=ac.用户ID)
            {
        ...
        public JsonResult GetAssociatedAuditor()
        {
            ...
            foreach (var item in models)
            {
                ...
                var Auditor = new string[] { item.用户ID.ToString(), item.姓名 ,item.部门};
                tripObj.Add(Auditor);
            }
            return Json(tripObj);
        }
        ...
        /// 构造接待数据
        /// </summary>
        /// <param name="role">当前角色（1为审核）</param>
        /// <returns></returns>
